...
When Scramjet asks you to report command status, call `report_report_scramjet_command_status` with `status: "completed"`. This command declares a `forced` next step, so Scramjet runs `mach12:pr-review-assessment` regardless; include a single `next_steps` entry only to pass the runtime context to that forced target:
