...
When Scramjet asks you to report command status, call `report_report_scramjet_command_status` with `status: "completed"` and include **both** declared candidates in `next_steps` so the user can see all options:
