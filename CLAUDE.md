...
When modifying command frontmatter schema (`commands/loader.ts`, `commands/parse-next-step.ts`), delegation behavior (`delegate.ts`), tool scoping (`delegate.ts`, `tool-scope-advisory.ts`), status reporting (`command-status.ts`), lifecycle state transitions (`phase-machine.ts`), or next-step dispatch (`auto-continue.ts`, `commands/validator.ts`), update `docs/command-authoring.md` to reflect the change. When modifying lifecycle state structure or transitions, also update `docs/lifecycle-state-space.md`. When modifying log categories, levels, or lifecycle event instrumentation, update `docs/logging.md`. The authoring doc is agent-facing - inaccurate guidance produces malformed commands.

When diagnosing harness misbehavior (probe didn't fire, command didn't chain, unexpected pause), consult `docs/logging.md` for the entry schema, `jq` query patterns, and diagnostic workflow.
...
`index.ts` constructs a single `ScramjetState` (registry, agent registry, delegate stack, sidebar log, `/scramjet on/off` flag, pending forced dispatch, `lifecycle: LifecycleState` - see `types.ts` and `phase-machine.ts`, `logger: ScramjetLogger` - see `logger.ts`) and threads it through every register-call. Each capability is its own file; nothing in the harness imports anything outside `types.ts` and Pi's API.
...
- `phase-machine.ts` - defines the `LifecycleState` discriminated union (`idle | dormant | running | probing | reported | waiting`), each variant carrying exactly the data its phase needs (command identity, status payload, continue count). Exports a pure `transition(state, event)` function with result-type return for all legal state transitions, plus replay-reconstruction helpers that restore only stable resting states (`waiting`, `dormant`) on resume. See `docs/lifecycle-state-space.md` for the full transition table and design rationale.
...
- `command-status.ts` + `auto-continue.ts` - the harness mechanism that drives next-step dispatch via a two-phase protocol. The command produces its normal user-facing answer first; nothing about completion is injected into that turn. `auto-continue.ts` listens on `agent_end`: when the lifecycle state narrows to `running` (an active top-level command with a `next:` policy), it transitions to `probing` via `phase-machine.ts` and **defers** (via `setTimeout(0)`, after the run settles and `isStreaming` clears when `agent.prompt()` resolves) a TUI-hidden status-check message built by `buildProbeMessage`, sent through `pi.sendMessage({ display: false }, { triggerTurn: true })` to start a short probe turn. `command-status.ts` registers the `report_scramjet_command_status` tool; its `execute` is harness-phase-gated - outside `probing` it returns a helpful error with no `terminate`; inside, it accepts `CommandStatusPayload`; `continuing` is non-terminating and transitions `probing → running` (incrementing `continueCount` on the lifecycle variant), while terminal statuses (`completed`/`blocked`/`incomplete`, plus `summary` and a `next_steps[]` array) are stored, journaled, advanced to `reported`, and returned with `terminate: true`. On the probe turn's `agent_end` (`reported`), `auto-continue.ts` routes by status: `completed` (→ `idle`) validates the pick via the policy and dispatches mode-by-mode (`forced` fires unconditionally even under `/scramjet off`; `closed`/`open` honor the first valid `next_steps` entry when `/scramjet on`; `ask` pauses); `blocked` (→ `idle`) warns; `incomplete` (→ `idle`) pauses quietly. `waiting` is one of two resting phases besides `idle` (the other is `dormant`): a command parked via `get_scramjet_user_input` (freetext or cancellation) stays associated with its invocation, and an **interactive, non-slash** reply (handled in `history.ts`'s `input` handler) transitions `waiting → running`, re-arming the existing `running → probing` probe so the resumed command can later report `completed` and offer its declared next step. Chaining still requires an explicit `completed` report, so an accidental resume can only re-probe - never mis-chain; a stray `agent_end` while `waiting` is a defensive no-op (no probe without a user reply); and exiting the workflow via an unknown slash drops `waiting → idle`. A `probing` `agent_end` with no report self-heals to `dormant` (not `idle`), preserving the command association so a later interactive reply can still recover the command (issue 128). On `rebuild` (resume / branch switch) the transient phases (`running`/`probing`/`reported`) are never restored, but the stable `waiting` and `dormant` halts are: `user-input.ts` journals `scramjet:user-input-parked` entries and `replayHistory` uses `phase-machine.ts` reconstruction helpers to restore `waiting` (when a parked entry exists for the active command) or `dormant` (when a command start is associated but no parked entry is active), so paused or self-healed commands survive `pi --resume`. Journaling all statuses is what makes a command that waited, was answered, then completed **without** chaining reconstruct to `idle` - never resurrected - since that path writes no subsequent command-start. A stale post-resume tool call (any non-`waiting` reconstruction) still hits the phase guard rather than mis-dispatching; only the phase is reconstructed, never the full status payload.
...
- `user-input.ts` - registers the `get_scramjet_user_input` tool, which lets agents request structured user input during command execution. Supports three interaction types: `confirm` (yes/no via `MultiLineSelectList`, distinguishing explicit No from Escape/cancel), `select` (structured options with descriptions and `recommended` index via `MultiLineSelectList`), and `freetext` (parks at `waiting` so the user replies in the standard editor). A custom `renderCall` displays the freetext prompt `message` in the tool row so the user can see the question alongside the parked result. A `renderResult` hook persists the prompt message and outcome in the tool-result row after completion or cancellation; for select interactions, the rendered result includes the full options array (labels and descriptions). Phase-gated to `running` and `probing`; returns a helpful non-terminating error outside those phases, and for confirm/select in non-TUI mode. Runtime-validates type-specific required fields (e.g., `options` for select, non-empty `message`). Confirm/select called during `probing` suspend the probe watchdog via `state.suspendProbeWatchdog()` before awaiting UI, then transition `probing → running` after the response so command work can continue and the next `agent_end` schedules a fresh probe. Confirm/select cancellation (Escape) returns `{ cancelled: true }` and terminates; freetext always returns `terminate: true`, journals a `scramjet:user-input-parked` entry when a top-level command is active, and records only the prompt. Each interaction is journaled as a `scramjet:user-input` custom entry type via `pi.appendEntry()`. Advertised via `promptSnippet` on the tool definition.
...
- `model-identity.ts` - tracks the active model's identity and injects it into agent context so commands can produce accurate GitHub attribution without relying on LLM self-knowledge. Registers `session_start` to capture the initial model, contributes a stable `# Model Identity` section via `systemPromptSection` (id `scramjet:model-identity`, content set once from the initial model — same string = same cache prefix) on every `before_agent_start`, and handles `model_select` events with a 500ms debounce to deliver change notifications via two lifecycle-appropriate paths: `input` transform (prepends `[scramjet] Model changed to: ...` before user text when idle/waiting/dormant) or `before_agent_start` message return (with "Please continue" when the agent is actively working). Probe-phase safety: skips message injection when lifecycle is `probing` to avoid interfering with the two-phase command-status protocol. On resume/fork/branch-switch, reconstructs `currentModel` and `modelHistory` from `ModelChangeEntry` session entries and detects model divergence (stores as pending input transform). State: `currentModel: ModelRecord | null` and `modelHistory: ModelRecord[]` on `ScramjetState`; debounce timer and pending flags are closure-local. Always-on (flag-independent). Exports `buildModelIdentityBlock` and `reconstructModelState` for testability.
- `subdir-context.ts` - lazy-loads `CLAUDE.md` and `AGENTS.md` from subdirectories when the `read` tool returns a successful result for a file in a subdirectory. Discovers intermediate directories between cwd and the read file's directory (inside-cwd reads, capped at `MAX_DEPTH=10`) or only the immediate target directory (outside-cwd paths: absolute paths outside cwd, `~/`-prefixed outside cwd, or relative escapes). Resolves realpaths for symlink safety (skips directories whose realpath falls outside cwd for inside-cwd paths), deduplicates by directory realpath. Injects discovered content as synthetic read tool call/result pairs via Pi's `context` event — the original read result is never modified, and synthetic IDs are derived from display paths. Discoveries are retained in `state.subdirDiscoveries` and re-injected on every `context` call until `session_compact` clears state. Discovery records are journaled (`scramjet:subdir-context-discovery` entries) and reconstructed on resume/fork/branch-switch via `session_start`/`session_tree` handlers with compaction reset and defensive schema guards. Bounded by `MAX_DIRS=20` (directory cap) and `MAX_DEPTH=10` (path depth cap). Error-discriminating: suppresses ENOENT silently, logs non-ENOENT errors via `state.logger`. Flag-independent (loads regardless of `/scramjet on|off`). Exports functions (`directoriesToCheck`, `discoverContextFiles`, `createStableId`, `reconstructSubdirState`, `findAnchorIndex`, `buildSyntheticPair`, `formatContextBlocks`) for testability.
...
- **Zero lock-in.** The user can press Escape at any transition and be back in normal Pi. No workflow state persists. Don't add resumable state, queues, or progress tracking across sessions. *Exception (issue 88 + issue 135 + issue 156): a command parked via `get_scramjet_user_input` (freetext or cancellation) sits at a stable `waiting` phase, and a probe self-heal parks at `dormant`; both are reconstructed on `pi --resume` / branch switch via `phase-machine.ts` replay helpers, so the user can answer and resume the in-flight command. These are the two authorized resumable halts — only the lifecycle phase and command identity are reconstructed (never the full status payload), the transient mid-turn phases (`running`/`probing`/`reported`) are deliberately not journaled, and a command that already completed reconstructs to `idle` and never re-fires. See the auto-continuation bullet in the architecture section, `docs/lifecycle-state-space.md`, and `docs/scramjet-vision.md` §2.1. Don’t generalize this into broader cross-session workflow persistence.*
