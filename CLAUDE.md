...
- `subdir-context.ts` - lazy-loads `CLAUDE.md` and `AGENTS.md` from subdirectories when the `read` tool returns a successful result for a file in a subdirectory. Discovers all intermediate directories between cwd and the read file's directory (inside-cwd reads) or only the immediate target directory (outside-cwd absolute reads). Resolves realpaths for symlink safety (skips directories whose realpath falls outside cwd for inside-cwd paths), deduplicates by realpath. Injects discovered content as synthetic read tool call/result pairs via Pi's `context` event — the original read result is never modified. Discoveries are retained in `state.subdirDiscoveries` and re-injected on every `context` call until `session_compact` clears state. Discovery records are journaled (`scramjet:subdir-context-discovery` entries) and reconstructed on resume/fork/branch-switch via `session_start`/`session_tree` handlers with compaction reset and defensive schema guards. Bounded by `MAX_DIRS=20` (directory cap) and `MAX_DEPTH=10` (path depth cap). Error-discriminating: suppresses ENOENT silently, logs non-ENOENT errors via `state.logger`. Flag-independent (loads regardless of `/scramjet on|off`). Exports pure functions (`directoriesToCheck`, `discoverContextFiles`, `createStableId`, `reconstructSubdirState`, `findAnchorIndex`, `buildSyntheticPair`, `formatContextBlocks`) for testability.
