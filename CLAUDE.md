...
- **Model switching / model-change communication** (issue 244) — three explicitly separated concerns, all always-on (flag-independent), all riding the harness-tool-invocation primitive (see the note below):
  - `model-switch-tool.ts` — registers the agent-callable `switch_scramjet_model` tool. The agent calls it to change the active harness model through the canonical selection path (`pi.setModel`); the tool's own tool row is the transcript record of the change (not a separate notice). `execute` resolves the target via `ctx.modelRegistry.find(provider, model)` — unknown model: soft-text error listing the `getAvailable()` catalog, with no `pi.setModel` call (no `model_select`, no history entry, model unchanged); resolved model: sets `state.suppressNextModelNotify = true` (read synchronously by the `model-change-notice.ts` handler, since `setModel` emits and awaits `model_select` before it resolves), then calls `pi.setModel`, reporting a `false` return (no auth) or a thrown call as soft-text errors that clear the flag and leave the model unchanged. Registered and advertised via `promptSnippet`, no confirmation, never a silent fallback.
