...
- `command-status.ts` + `auto-continue.ts` — the harness mechanism that drives next-step dispatch via a two-phase protocol. The command produces its normal user-facing answer first; nothing about completion is injected into that turn. `auto-continue.ts` listens on `agent_end`: when an active top-level command with a `next:` policy goes idle (`commandPhase === "running"`), it advances the phase to `probing` and **defers** (via `setTimeout(0)`, after `finishRun()` clears `isStreaming`) a TUI-hidden status-check message built by `buildProbeMessage`, sent through `pi.sendMessage({ display: false }, { triggerTurn: true })` to start a short probe turn. `command-status.ts` registers the `scramjet_command_status` tool; its `execute` is harness-phase-gated — outside `probing` it returns a helpful error with no `terminate`; inside, it stores the `CommandStatusPayload` (`status` ∈ `completed`/`waiting_for_user`/`blocked`/`incomplete`, plus `summary`, optional `user_prompt`, and a `next_steps[]` array), advances the phase to `reported`, and returns `terminate: true`. On the probe turn's `agent_end` (`reported`), `auto-continue.ts` routes by status: `completed` validates the pick via the policy and dispatches mode-by-mode (`forced` fires unconditionally even under `/scramjet off`; `closed`/`open` honor the first valid `next_steps` entry when `/scramjet on`; `ask` pauses); `blocked` warns; `waiting_for_user` and `incomplete` pause quietly. A `probing` `agent_end` with no report self-heals to `idle` (no loop). The phase is reset to `idle` on `rebuild` (resume / branch switch), so a stale post-resume tool call hits the phase guard rather than mis-dispatching.
