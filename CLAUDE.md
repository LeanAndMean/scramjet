...
- `model-identity.ts` — tracks the active model's identity and injects it into agent context so commands can produce accurate GitHub attribution without relying on LLM self-knowledge. Registers `session_start` to capture the initial model, contributes a stable `# Model Identity` section via `systemPromptSection` (id `scramjet:model-identity`, content set once from the initial model — same string = same cache prefix) on every `before_agent_start`. Delivers model change notifications as `notify_model_change` tool calls via three delivery paths: (1) `before_agent_start` returns `preTurnMessages` (between-turns delivery — pending change exists at the start of a new agent run); (2) `message_end` mutates the assistant message to append a tool call (mid-run delivery — change arrives during an agent run with tool calls, injected alongside existing tool calls); (3) `prepare_next_turn` returns `messages` (intra-run delivery — change arrives during tool execution, injected before the next LLM call). All three paths are gated behind a 500ms stability wait (`waitForModelStable`, capped at `MAX_STABILITY_WAIT_MS` = 5s) and a probe-safety guard (`probeInFlight`). Pre-first-turn model changes update the system prompt section directly (no tool call). On resume/fork/branch-switch, reconstructs `currentModel` and `modelHistory` from session entries and detects model divergence (stores as `pendingModelChange`). Reconstruction filters synthetic assistant messages (`provider: "scramjet"`) from turn index estimation. State: `currentModel: ModelRecord | null`, `modelHistory: ModelRecord[]`, `pendingModelChange: ModelRecord | null`, and `lastModelSelectTime: number` on `ScramjetState`. Always-on (flag-independent). Exports `buildModelIdentityBlock`, `buildNotificationToolCall`, `buildNotificationPair`, `reconstructModelState`, `waitForModelStable`, `STABILITY_MS`, and `MAX_STABILITY_WAIT_MS` for testability.
