...
- `subdir-context.ts` - lazy-loads `CLAUDE.md` and `AGENTS.md` from subdirectories when the `read` tool returns a result for a file inside cwd. Discovers all intermediate directories between cwd and the read file's directory, resolves realpaths for symlink safety (skips directories whose realpath falls outside cwd), deduplicates by realpath, and prepends discovered content to the tool result. Bounded by `MAX_DIRS=20` (directory cap) and `MAX_DEPTH=10` (path depth cap). Error-discriminating: suppresses ENOENT silently, logs non-ENOENT errors via `state.logger`. State clears on `session_compact` for re-discovery. Flag-independent (loads regardless of `/scramjet on|off`). Exports pure functions (`directoriesToCheck`, `discoverContextFiles`, `formatContextBlocks`) for testability.
