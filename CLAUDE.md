...
- `diagram/` — custom Mermaid renderer (parser + integer-grid layout + A* edge routing). Supports flowchart/graph and stateDiagram-v2. Theme colors applied via `CharRole` → `ThemeColor` mapping at render time. No external dependencies. Registration skipped at runtime (issue 232); implementation preserved for future improvement.
