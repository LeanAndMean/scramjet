...
1. `"agent_end observed"` — agent turn ended, lifecycle phase checked
2. `"status probe preparing"` — phase is `running`, probe eligible
3. `"status probe scheduled"` — deferred probe timer set
4. `"status probe timer fired"` — timer callback ran
5. `"probe watchdog armed"` — watchdog timeout set for probe turn
6. `"status probe sent"` — `sendMessage` succeeded
7. `"status report accepted"` — `report_scramjet_command_status` called with valid payload
8. `"lifecycle transition"` — phase transition (e.g., `probing → reported`)
9. `"probe watchdog cleared"` — watchdog cancelled (report received in time)
10. `"next-step policy evaluated"` — policy mode determined
11. `"next step dispatching"` or `"next-step dispatch skipped"` — dispatch decision

### Probe failure patterns

**Watchdog timeout** (probe turn never reported):
- Entries 1–6 present, then:
- `"probe watchdog fired"` — timeout elapsed without status report
- warn: `"status probe turn never completed; auto-continue paused"`

**Probe send failure**:
- Entries 1–4 present, then:
- warn: `"status probe failed to send"`

**No valid report on probe turn end**:
- Entries 1–6 present, watchdog not fired, then:
- `"agent_end observed"` (phase = `probing`)
- warn: `"status probe turn ended without a valid status report; auto-continue paused"`
- `"lifecycle transition"` (probing → dormant, self-heal)

**Phase-gate rejection** (tool called out of phase):
...
5. **Check phase transitions** — filter for `"lifecycle transition"` messages:
   ```sh
   jq -c 'select(.type == "scramjet:log" and .data.message == "lifecycle transition") | .data.data' session.jsonl
   ```
...
| Probe never fired | No `"status probe scheduled"` after `"agent_end observed"` | Phase wasn't `running` at agent_end |
...
| Double agent_end | Two `"agent_end observed"` without intervening probe | Fast successive turns; second skipped by phase guard |
| Self-heal to dormant | `"lifecycle transition"` with `to: "dormant"` | Probe ended without report; command preserved for resume |
