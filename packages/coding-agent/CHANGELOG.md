...
- Custom `models.json` schema support for Anthropic Messages `supportsTemperature` and `forceAdaptiveThinking` compat fields used by the Opus 4.8 / Fable 5 / Sonnet 5 model support update.
...
- Default Anthropic model resolution now selects `claude-opus-4-8`; default Amazon Bedrock model resolution now selects `us.anthropic.claude-opus-4-8`.
