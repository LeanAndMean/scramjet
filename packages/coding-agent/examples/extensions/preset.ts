 ...
 * - ~/.scramjet/agent/presets.json (global)
 * - <cwd>/.scramjet/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `scramjet --preset plan` - start with plan preset
...
import { DynamicBorder, getAgentDir } from "@leanandmean/coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@leanandmean/tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".scramjet", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.scramjet/agent/presets.json or .scramjet/presets.json", "warning");
			return;
		}
