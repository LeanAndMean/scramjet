...
LLMs have limited context windows. When conversations grow too long, Scramjet uses compaction to summarize older content while preserving recent work. This page covers both auto-compaction and branch summarization.
...
For TypeScript definitions in your project, inspect `node_modules/@leanandmean/coding-agent/dist/`.

## Overview

Scramjet has two summarization mechanisms:
...
Explicit output limits include thinking in the Anthropic and Bedrock simple adapters. Too little space for minimum token-based thinking produces an actionable error, not silent reasoning disablement. Codex does not serialize an output limit, so its output allocation is not wire-enforced. OpenRouter ordinary requests omit an implicit aggregate output maximum to avoid excluding long-context routes; explicit summary limits remain bounded. When provider-evidenced `requestLimits` are present, both allocation implementations keep each endpoint's total/input/output constraints together and require tool support for requests containing tools. Explicit output is capped by the largest compatible endpoint allowance, including Vercel's required output field; no compatible entry produces an actionable error. Scalar context/output maxima and reserve accounting remain unchanged. See [models.md](models.md#joint-endpoint-constraints) for metadata validation and the remaining custom-routing, capability, and account qualifications.
...
On repeated compactions, the summarized span starts at the previous compaction's kept boundary (`firstKeptEntryId`), not at the compaction entry itself, falling back to the entry after the previous compaction if that kept entry cannot be found in the path. This preserves messages that survived the earlier compaction by including them in the next summarization pass as well. Scramjet also recalculates `tokensBefore` from the rebuilt session context before writing the new `CompactionEntry`, so the token count reflects the actual pre-compaction context being replaced.
...
For split turns, Scramjet generates two summaries and merges them:
...
When you use `/tree` to navigate to a different branch, Scramjet offers to summarize the work you're leaving. This injects context from the left branch into the new branch.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, Scramjet extracts file operations from:
...
```typescript
import { convertToLlm, serializeConversation } from "@leanandmean/coding-agent";
...
Configure compaction in `~/.scramjet/agent/settings.json` or `<project-dir>/.scramjet/settings.json`:
