...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.scramjet/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.scramjet/extensions/`)
- Project skills:
  - `.scramjet/skills/`
...
For each built-in provider request, `createAgentSession()` binds the current extension runner when `before_provider_call` begins, then carries that runner through provider-neutral preparation, serialization, payload rewriting, and observation. It also captures a request-local copy of provider-neutral tool names, a bounded routed-model identity (`provider`, `id`, and `api` only), and the run-captured public `Agent.onPayload` callback without changing that callback's signature. Payload processing order is public callback → captured runner's `before_provider_request` chain → immutable `provider_request_tool_inventory` observation → transport. The public callback runs exactly once; `undefined` keeps its input unchanged, while any other return value becomes the input to extension handlers. If successful reload supersedes the captured runner before payload dispatch, the public callback still runs but the old request skips extension rewriting and observation rather than entering the replacement runtime; if replacement occurs while the old runner's chain is already in flight, its completed replacement reaches transport and only observation is suppressed. The wrapper inspects only tool names from the actual final payload and returns that exact same value to transport. Known malformed shapes expose only a bounded reason, custom APIs are `unsupported`, and custom providers that never invoke `onPayload` provide no final-schema proof. The observation does not prove transport or model behavior; see [extensions.md](extensions.md#provider_request_tool_inventory). Because this adds a member to the exported `ExtensionEvent` union, exhaustive TypeScript switches must handle it or include a default/unknown-event branch.
...
// Default: uses ~/.scramjet/agent/auth.json and ~/.scramjet/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.scramjet/agent/extensions/`, `.scramjet/extensions/`, and settings.json extension sources.
...
1. Global: `~/.scramjet/agent/settings.json`
2. Project: `<cwd>/.scramjet/settings.json`
