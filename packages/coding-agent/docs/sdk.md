...
For each built-in provider request, `createAgentSession()` captures a request-local copy of provider-neutral tool names, a bounded routed-model identity (`provider`, `id`, and `api` only), and the current extension runner, then wraps the existing payload callback without changing its public signature. After all `before_provider_request` handlers have run, the wrapper inspects only tool names from the final payload and emits the immutable, observation-only `provider_request_tool_inventory` extension event before returning the exact same final payload value to transport. If successful reload replaces that runner while payload rewriting is in flight, the old request's observation is suppressed rather than delivered to the replacement runtime. Known malformed shapes expose only a bounded reason, custom APIs are `unsupported`, and custom providers that never invoke `onPayload` provide no final-schema proof. The observation does not prove transport or model behavior; see [extensions.md](extensions.md#provider_request_tool_inventory). Because this adds a member to the exported `ExtensionEvent` union, exhaustive TypeScript switches must handle it or include a default/unknown-event branch.
