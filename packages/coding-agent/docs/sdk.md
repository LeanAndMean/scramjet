...
For each built-in provider request, `createAgentSession()` captures a request-local copy of provider-neutral tool names, a bounded routed-model identity (`provider`, `id`, and `api` only), and the current extension runner, then wraps the existing payload callback without changing its public signature. The captured runner owns that request's `before_provider_request` chain: if successful reload replaces it before payload dispatch begins, the old request skips both rewriting and observation rather than entering the replacement runtime; if replacement occurs while its chain is already in flight, only the old request's observation is suppressed. After all applicable handlers have run, the wrapper inspects only tool names from the final payload and emits the immutable, observation-only `provider_request_tool_inventory` extension event before returning the exact same final payload value to transport. Known malformed shapes expose only a bounded reason, custom APIs are `unsupported`, and custom providers that never invoke `onPayload` provide no final-schema proof. The observation does not prove transport or model behavior; see [extensions.md](extensions.md#provider_request_tool_inventory). Because this adds a member to the exported `ExtensionEvent` union, exhaustive TypeScript switches must handle it or include a default/unknown-event branch.
