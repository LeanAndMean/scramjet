...
Default production activation, approval-context flush migration, final-exit transcript output, and application suspend/external-editor handoffs remain pending. The private production candidate still rejects committed-only approval flushing; do not use it for real approval workflows yet. Built-in Kitty/iTerm2 placements are re-anchored to absolute rows only when their full extent fits; clipped spans show a placeholder. Base images are withheld with a placeholder while overlays are visible. Oversized-image fitting, selection with graphics, custom graphics envelopes, and native graphics verification remain pending; do not use the candidate as a production graphics replacement yet.
