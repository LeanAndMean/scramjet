...
GitHub Copilot catalog visibility requires configured Copilot authentication, but a listed model is not proof that the current account is entitled to use it or that the endpoint will accept a request. Copilot prices are scalar usage estimates for a subscription route, not guaranteed marginal charges. Grok's separate long-context rates cannot be represented by the scalar estimator, so estimates for long requests may be too low while token counts remain valid.
