...
```
~/.scramjet/agent/sessions/--<path>--/<timestamp>_<uuid>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.scramjet/agent/sessions/`.

Scramjet also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, Scramjet uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/@leanandmean/coding-agent/dist/` and `node_modules/@leanandmean/ai/dist/`.
...
### Base Message Types (from `@leanandmean/ai`)
...
interface AssistantMessage {
  ...
  origin?: "provider" | "harness";  // Explicit for new messages; absent in legacy sessions
  api: string;
  provider: string;
  model: string;
  usage: Usage;
...
{"type":"message","id":"b2c3d4e5","parentId":"a1b2c3d4","timestamp":"2024-12-03T14:00:02.000Z","message":{"role":"assistant","content":[{"type":"text","text":"Hi!"}],"origin":"provider","provider":"anthropic","model":"claude-sonnet-4-5","usage":{...},"stopReason":"stop"}}
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if scramjet-generated (legacy field name)
