...
After editing `keybindings.json`, run `/reload` in Scramjet to apply the changes without restarting the session.
...
Create `~/.scramjet/agent/keybindings.json`:
...
On native Windows, `app.suspend` has no default binding because Windows terminals do not support Unix job control. If you bind it manually, Scramjet shows a status message instead of suspending. In WSL, the normal Linux `ctrl+z`/`fg` behavior still applies.
