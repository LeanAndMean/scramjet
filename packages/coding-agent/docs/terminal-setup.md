...
During an interactive session, Scramjet owns the alternate screen, mouse reporting, the **rightmost transcript scrollbar**, and drag selection. Wheel/trackpad input and scrollbar dragging browse retained output, including running cards. The terminal's native scrollbar/history is not the live transcript. Returning to the document bottom resumes following; passive updates do not pull you away from what you are reading. The input area is docked by default; typing there preserves the transcript anchor. `/settings` can change docking, the input-text height ceiling (30%, range 10–50%), and wheel step (3, range 1–20). Input shrinks around adjacent widgets before an oversized band falls back to explained, undocked retained browsing. Selection remains live during output and layout changes. Endpoints track component-local positions on a best-effort basis; the displayed highlight, not an earlier intended range, determines copying.

The leading dock gap shows `Session: N lines below · Ctrl+End: latest` when rendered session rows remain below the viewport. It uses existing whitespace, not another rule or the editor's hidden-draft indicators, and returns to a blank row at the tail. Narrow layouts shorten the hint while retaining **Session**, or show `Session ↓…` when the exact number cannot fit. It is not an unread count and is excluded from application copying. Scramjet choice controls have paired rules and row-based selection-following windows; partial neighboring choices remain visible, while arrows still select whole options. These changes do not alter selector cancellation, countdowns, transcript wheel ownership or approval safeguards.

Select by ordinary dragging, including across a screen edge. After starting a drag, keep the left button held and use the wheel to extend the transcript selection, either with a stationary pointer or while adjusting its horizontal and vertical position; reversing the wheel adjusts the endpoint. Release before browsing if you want to keep the selection unchanged. A press without an initial drag does not start wheel selection. Drags cross between transcript and dock in either direction: a transcript-origin downward drag autoscrolls to the bottom before entering the dock; a dock-origin upward drag selects the displayed transcript portion without including the hidden gap below it. Wheel scrolling does not extend a selection confined to the dock. Right-click a nonempty selection or use `tui.input.copy` (Ctrl+C by default) to copy displayed text without ANSI controls or scrollbar cells. Built-in text, Markdown and editor copying omit known component padding and rejoin soft wraps, retaining genuine indentation, hard line breaks and structural boundaries. Tables, repeated quote prefixes and unannotated custom renderers retain their physical layout rather than guessing at their source. Output and waiting controls continue updating while selected. Copy snapshots the last painted selection immediately, before clipboard delivery; unpainted pointer changes cannot change that copy. A new selection must have painted before it can be copied. Resizing ends an active drag and remaps its selection; successful copy or Escape clears selection. Copy failures retain the selection and temporarily display a diagnostic on the last screen row without changing geometry; clicking that diagnostic dismisses it without selecting the covered text. Without selection, Ctrl+C retains its normal application behavior. Right-clicking the visible, focused editor reads local clipboard text and inserts it at the caret without submitting. Other regions and selectors do not paste. Repeated eligible right-clicks during one clipboard read share that read; only the latest still-current request inserts text. Other intervening input, editing or session changes cancel a pending paste. WSL reads the Windows clipboard through PowerShell; macOS, Wayland/X11 and Termux use their local backends. Unsupported or remote clipboard reads show an actionable error; use your terminal's Paste command instead.
