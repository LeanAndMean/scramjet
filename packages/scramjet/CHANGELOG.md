...
- Add GPT-6 Sol, GPT-6 Luna, and Claude Opus 5.5 Copilot model contracts while preserving Claude Opus 5.

### Changed

- Serialize explicit supported Responses `off` selections through each model's declared `none` effort mapping.
- Document provider evidence boundaries for the new Copilot offerings.

### Tests

- Add generated-catalog, request-serialization, authenticated-registry, CLI-listing, and selector coverage for the new models and reasoning behavior.
...
- Show the waiting title while structured confirm/select prompts, forge publication approvals, and project-agent confirmations are unresolved.
- Restore the correct running or lifecycle-derived title after choices settle, including cancellation, stale-session, failure, and overlapping-choice paths.

### Tests

- Add regression coverage for interactive choice leases, title restoration, overlapping choices, cancellation, stale state, and failures.
...
- Add provider-boundary tool inventory inspection and privacy-safe Scramjet visibility diagnostics.
- Expose extension and SDK hooks for observing final serialized provider tool inventories.

### Changed

- Refresh live prompt and executable tool context between provider requests while preserving per-run composition and explicit callback overrides.
- Keep model-callable tools visible when invocation eligibility changes, while retaining harness-only exclusions.

### Tests

- Add provider serialization, lifecycle, reload, runtime replacement, prompt composition, and tool-visibility regression coverage.
...
- Link same-repository and cross-repository issue and pull-request references across Mach 12 artifact-authoring workflows.
- Preserve single-issue closure semantics, immutable comment references, and validation provenance formats.

### Tests

- Add contract coverage for reference formatting in general and executable-validation artifact workflows.
