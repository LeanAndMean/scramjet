...
`read_issue` and `read_pr` return filtered native `gh`/`glab` command transcripts. Provider field names, nesting, and JSON escaping differ by forge and remain untrusted evidence. GitLab comment segments exclude system, positioned, diff, and discussion notes so they represent only the supported top-level conversation. Omit `include` for the standard first-read segments; use exact segment subsets such as `include: ["comments"]` for refreshes. Follow each continuation notice with its returned segment list, item `offset`, optional oversized-item `byte_offset`, and unchanged global snapshot. Continuations are local fetch-once projections and do not rerun provider commands.

Comment creation requires complete earlier `artifact` and `comments` segments. Artifact edits require complete `artifact` evidence; comment edits require complete `comments` evidence. Segment snapshots can combine across calls, and cross-segment consistency is deliberately not claimed. Edit replacements match standard JSON-decoded provider field values exactly—do not copy visible JSON escape syntax when the intended value is a decoded tab or newline. Same-assistant-batch reads cannot authorize sibling mutations, successful or ambiguous writes invalidate only their affected segment, and compaction invalidates older evidence.

Mutation tools verify canonical identity and remote postimages but return only the canonical URL as model-visible success text; reread the stale segment when later work needs fresh content. Failures classify no-write reads/preflight, conclusive no-dispatch rejection, and ambiguous writes. After a surfaced read failure, a command with `bash` in scope may deliberately use read-only `gh`/`glab` inspection; no fallback is automatic. Ambiguous writes permit inspection only—never retry or a CLI mutation substitute. Approval remains command-owned—the tools establish safe transport and verification, not user authorization.

Issue relationships and readiness facts now use provider-native dialects rather than Scramjet-normalized records. Commands must branch explicitly between GitHub and GitLab field shapes, compare repository identity before passing bare child numbers to current-repository mutations, and obtain narrow provider evidence when a native object reply does not establish a readiness predicate. For GitLab draft PR creation, commands must obtain approval for an already draft-prefixed exact title because `create_pr` never rewrites approved content.

See [Forge Content Tools](forge-tools.md) for the public schemas, native transcript contract, segment evidence, exact replacement rules, queueing, failure semantics, renderer behavior, and deliberate exclusions.
...
Confirm and select return `{ "cancelled": true }` with `terminate: true` when the user presses Escape. Cancellation is not an error: Scramjet transitions the command to cancellation-resumable dormancy and records a dedicated durable grant. The next interactive non-slash reply consumes that grant and arms the normal completion-probe path; this does not revive blocked, incomplete, watchdog, aborted, or other generic dormant commands. A known slash command preserves the grant, a registered command start supersedes it, and a truly unknown slash exits the workflow. The command may also report `continuing` or a terminal status directly while dormant. Freetext remains a separate parked-input path.
...
When an interactive non-slash reply consumes a parked command, Scramjet resumes it live and journals a second `scramjet:user-input-parked` entry with `{ commandName, parked: false }`. This custom outcome does not duplicate the reply text; the editor reply remains a normal persisted user message. Legacy park entries written before this flag existed omit `parked` and are treated as `parked: true`.
...
- Don't rely on handling `{ "cancelled": true }` in the same turn — cancellation terminates the turn. If the user continues with an interactive non-slash reply, Scramjet resumes the cancellation-origin command automatically; unrelated generic dormant commands remain inert.
