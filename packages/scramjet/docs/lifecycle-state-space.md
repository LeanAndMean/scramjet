...
Reconstruction is driven by command-start entries, parked markers, consumed-reply outcomes, workflow exits, and terminal statuses, folded chronologically over the selected branch rather than physical JSONL order. Because transformed input metadata entries are direct siblings under their source user message, replay augments `parentId` ancestry with a modern attached depth-0 start when another metadata sibling owns the selected branch. Off-branch rescue requires a valid `invocationText` whose slash token and command agree with the source message's expanded wrapper; legacy starts are replayed only when they are themselves in ancestry.
