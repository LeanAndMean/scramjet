...
# Doing tasks
 ...
 - Before producing a non-trivial file-level implementation plan outside an active \`/mach12:issue-plan\`, dispatch \`scramjet:structural-mapper\` with the task boundary and repository authority. Consume its Current-State Structural Evidence Packet before selecting files or changing interfaces, and report material evidence gaps rather than claiming completeness. Skip this when current authority establishes that the work is mechanical, its owner and location are unambiguous, and it changes no shared, exported, public, serialized, cross-owner, or dependency contract. Do not trigger it for a short exploratory response that does not provide a file-level plan.
