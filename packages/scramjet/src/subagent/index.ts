...
async function writePromptToTempFile(agentName: string, prompt: string): Promise<{ dir: string; filePath: string }> {
	const tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "scramjet-subagent-"));
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				const emitParallelUpdate = () => {
					if (onUpdate) {
						const running = allResults.filter((r) => r.exitCode === EXIT_CODE_RUNNING).length;
						const done = allResults.filter((r) => r.exitCode !== EXIT_CODE_RUNNING).length;
						onUpdate({
							content: [
