...
describe("next-step selection journaling", () => {
	beforeEach(() => vi.useFakeTimers());
	afterEach(() => vi.useRealTimers());
	...
	it("journals outcome=selected on completion selection", async () => {
		...
		const entries = selectionEntries(bag.pi);
		expect(entries).toHaveLength(1);
		expect(entries[0].outcome).toBe("selected");
		expect(entries[0].source).toBe("completion");
		expect(entries[0].selected).toEqual({ message: "/b:ok", reason: "continue" });
	});

	it("journals outcome=dismissed on completion dismiss", async () => {
		const def = defWithPolicy("a:cmd", { mode: "closed", candidates: [{ name: "b:ok" }] });
		const state = runningState(def, { enabled: false });
		const { bag, ctxBag, report } = bootstrap(state);
		...
		const entries = selectionEntries(bag.pi);
		expect(entries).toHaveLength(1);
		expect(entries[0].outcome).toBe("dismissed");
		expect(entries[0].source).toBe("completion");
		expect(entries[0].selected).toBeNull();
		expect(entries[0].modelSwitch).toBeNull();
	});

	it("populates policyMode from the completed command's policy", async () => {
		const def = defWithPolicy("a:cmd", { mode: "open", candidates: [] });
		const state = runningState(def, { enabled: false });
		const { bag, ctxBag, report } = bootstrap(state);
		...
		const entries = selectionEntries(bag.pi);
		expect(entries).toHaveLength(1);
		expect(entries[0].policyMode).toBe("open");
	});

	it("journals source=suggestion for suggestion selections", async () => {
		...
		const entries = selectionEntries(bag.pi);
		expect(entries).toHaveLength(1);
		expect(entries[0].source).toBe("suggestion");
		expect(entries[0].policyMode).toBeNull();
		expect(entries[0].outcome).toBe("selected");
	});

	it("journals outcome=dismissed for suggestion dismiss", async () => {
		const steps = [{ message: "/mach12:pr-review 248", reason: "review" }];
		const state = freshState({
			enabled: true,
			lifecycle: lifecycleFor("idle"),
	...
	it("captures model cycling in the entry", async () => {
		const ARROW_RIGHT = "\x1b[C";
		const modelA = { provider: "anthropic", id: "claude-opus-4", name: "Claude Opus 4" };
		const modelB = { provider: "openai", id: "gpt-5.5", name: "GPT-5.5" };
		...
		const def = defWithPolicy("a:cmd", { mode: "closed", candidates: [{ name: "b:ok" }] });
		const state = runningState(def, { enabled: false });
		const { bag, ctxBag, report } = bootstrap(state, { hasUI: true, models: [modelA, modelB], model: modelA });
		...
		const entries = selectionEntries(bag.pi);
		expect(entries).toHaveLength(1);
		expect(entries[0].modelSwitch).toBe("GPT-5.5");
	});

	it("stale guard prevents journaling", async () => {
		const def = defWithPolicy("a:cmd", { mode: "closed", candidates: [{ name: "b:ok" }] });
		const state = runningState(def, { enabled: true });
		const { bag, ctxBag, report } = bootstrap(state);
		let resolveSelector: (value: string | null) => void = () => {};
	...
	it("captures full entry shape (sourceCommand, options, recommendedIndex)", async () => {
		const def = defWithPolicy("a:cmd", { mode: "closed", candidates: [{ name: "b:ok" }, { name: "c:alt" }] });
		const state = runningState(def, {
			enabled: false,
			registry: registryWith(defWithPolicy("c:alt", undefined)),
		});
		const { bag, ctxBag, report } = bootstrap(state);
		...
		await simulateTwoTurns(bag, ctxBag, report, {
			...
			summary: "done",
			next_steps: [
				{ message: "/b:ok", reason: "primary path" },
		...
		expect(entry.sourceCommand).toBe("a:cmd");
		expect(entry.options).toEqual([
			{ message: "/b:ok", reason: "primary path" },
			{ message: "/c:alt", reason: "alternate" },
		]);
		expect(entry.recommendedIndex).toBe(1);
		expect(entry.selected).toEqual({ message: "/c:alt", reason: "alternate" });
	});
});
