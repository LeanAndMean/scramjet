...
describe("mach12 ordinary PR readiness", () => {
	...
	it("merge reacquires repository release authority before informed approval", () => {
		...
		expect(release).toMatch(/any mandatory[^.]*preflight/i);
		expect(release).toMatch(/when none applies[^.]*rather than inventing one/i);
	...
	it("merge rechecks release conflicts and preflight immediately before explicit-target creation", () => {
		...
		const preflight = release.indexOf("applicable authority-defined read-only preflight", head);
