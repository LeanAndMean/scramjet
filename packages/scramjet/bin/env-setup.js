...
// "@scramjet/coding-agent"` resolves; ESM evaluates imports in
...
// just resolve `@scramjet/coding-agent/package.json`. Resolve
...
const piEntryUrl = import.meta.resolve("@scramjet/coding-agent");
const acceptedPiPackageNames = new Set(["@scramjet/coding-agent", "@leanandmean/pi-coding-agent"]);
...
while (true) {
	...
	if (parent === piRoot) {
		throw new Error(
			`Could not locate @scramjet/coding-agent package root from ${piEntryUrl}; expected package name ${Array.from(acceptedPiPackageNames).join(" or ")}`,
		);
	}
	piRoot = parent;
}
