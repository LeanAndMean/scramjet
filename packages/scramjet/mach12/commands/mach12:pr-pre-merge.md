...
Use `read_pr` with `include: ["artifact", "comments"]` before changing the branch and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot. Read provider-native object facts—GitHub uses `state`, `draft`, `mergeable`, `head.ref`, and `base.ref`; GitLab uses `state`, `draft`, `detailed_merge_status`, `source_branch`, and `target_branch`. Require the PR to be open and non-draft. Because the native object does not consistently establish requested-change or required-review state, use a narrow provider query and require conclusive evidence; otherwise report incomplete rather than treating an absent field as review-clear. Retain a narrow `gh pr view <pr-number> --json mergeStateStatus` query for GitHub branch-freshness/conflict state, and read required checks with `gh pr checks <pr-number> --required`. If GitHub reports `CONFLICTING` or `DIRTY`, retain that conflict evidence and continue through checkout to Step 5 for guarded remediation rather than blocking immediately. This route does not bypass Step 5's Merge/Cancel choice and does not authorize an automatic merge. A behind branch continues to Step 5, and pending or failing checks continue to Step 9 for resolution rather than blocking the checklist immediately. If mergeability remains unknown after one brief reread with `read_pr`, report incomplete rather than guessing.
...
1. **PR content and changed-file metadata**: use `read_pr` with `include: ["artifact", "files"]` and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until both native segments are complete.
