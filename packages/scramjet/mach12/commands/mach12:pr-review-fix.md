...
1. **Discovery and scope/requirements** -- restate the goal: fix the selected findings only; do not fix other findings in the review. The selected finding set is fixed after Step 3 unless the user explicitly revises it. Requirement clarification may resolve meaning within that set but must not silently add, remove, merge, defer, or reclassify findings.

   Self-assess the authenticated authoritative review, optional assessment, validation proofs, user context, and current repository evidence before asking questions. Ask scope and requirements questions only when those authorities do not establish the required behavior, bounded scope, compatibility constraints, acceptance behavior, or user-owned edge cases. Do not ask ceremonial questions when the answer is already established.
2. **Codebase exploration** -- read every file referenced by the selected findings and trace the relevant code paths. When more context is needed, dispatch focused `mach12:code-explorer` tasks for the selected findings only. Verify current assumptions before architecture analysis.
3. **Proportional architecture analysis** -- after requirements are established and relevant codebase exploration is complete, classify the selected work by its unresolved architectural decision boundaries:
   - **Trivial:** no material structural choice exists; bypass architect dispatch.
   - **Authoritatively settled:** the authoritative review, optional assessment, applicable user decisions, or verified repository evidence establishes a sound existing approach; follow it without architect or option ceremony.
   - **Unresolved non-trivial:** material choices remain in responsibility, interfaces, state, ordering, data flow, dependencies, test seams, or correctness-required refactoring; this requires one or more `mach12:code-architect` subagents.

   For unresolved non-trivial work, create the minimum number of briefs needed for independent architectural decision boundaries. Combine findings governed by the same decision, and dispatch independently decidable briefs together in one parallel batch. Do not use finding count, file count, or line count as a threshold, and do not manufacture extra lenses.

   Every architect brief must include the authoritative review and optional assessment, exact selected findings and descriptions, current code observations, applicable user decisions, repository constraints, validation-proof restrictions, and explicit non-goals. Request one minimum-sufficient design covering affected files, ownership, data flow, test implications, and rejected unnecessary machinery. Architects are advisory: they must not expand or reinterpret selected findings, choose the final design, mutate the repository, publish, or push.

   Treat architect recommendations as evidence. The parent rejects scope expansion, proof weakening, unsupported abstractions, and stale assumptions, then selects or synthesizes the smallest supported design. Ask architecture questions only after exploration and architect synthesis, only when repository evidence cannot safely select an approach and a consequential preference-dependent trade-off remains. If architecture analysis exposes a true requirements contradiction, return explicitly to scope and requirements clarification rather than silently reopening scope.
4. **Implementation** -- write the code only after the applicable requirement and architecture completion conditions above are satisfied. The parent owns the final design, repository mutation, and testing; follow existing codebase conventions strictly.
