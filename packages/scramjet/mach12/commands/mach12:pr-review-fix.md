...
6. **Quality review** -- run a single, lightweight sanity pass over the selected findings' changes. This is a fast confidence check that the fix diff is sound; it is explicitly **not** a substitute for the thorough, full-branch PR review that runs later. Classify the changed surfaces before dispatch:
   - For command-only fixes, use one `scramjet:command-reviewer` when a material independent review is warranted. Use `scramjet:instruction-semantics-analyzer` instead for a narrow wording or contract question; do not dispatch both for overlapping review.
   ...
   - **Cap: at most 3 subagents per stage, total across both families** -- dispatch independent briefs in one parallel batch. Three is a ceiling for unusually risky mixed or code fixes, not a quota; command-only work normally uses at most one, and trivial work may skip review.
