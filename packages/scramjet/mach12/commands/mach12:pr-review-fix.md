...
Before treating either comment as an executable-validation artifact, require both exact comment IDs and both SHA-256 bindings. Resolve and freeze the authenticated GitHub login. Fetch full comment metadata and verify repository/PR ownership, that both artifact authors exactly equal the authenticated login with `OWNER`, `MEMBER`, or `COLLABORATOR` association, that the review's recorded publisher login agrees, the expected `<!-- mach12-review -->` and `<!-- mach12-assessment -->` markers, exact body digests, and that the assessment links the exact review ID and records its matching digest. Require the trusted review and assessment artifacts to refer to the same PR, frozen implementation parent `P`, original proof commit `V`, recorded actual merge base, proof paths, node IDs, per-path proof content SHA-256 identities, findings, and exhaustive non-overlapping ownership-group mapping. Require those immutable review-time identities to agree across both artifacts. Source the current assessment head exclusively from the authenticated assessment artifact; the earlier review artifact cannot know a post-assessment cleanup successor. Recompute each content identity from the exact committed blob bytes at `V`; artifact prose alone is not content authority.

For the first repair session, independently require the PR remains open, the local non-detached feature branch matches the PR head branch, local `HEAD`, its upstream branch, and a fresh GitHub `headRefOid` equal the authenticated current assessment head. Require `V` to have exactly one parent equal to `P`, `V` to remain an ancestor of the authenticated current head without unexpected merge commits or forks, and the recorded actual merge base to remain valid. Require a clean index and tracked and untracked worktree, and verify every surviving committed node remains at its authenticated path with unchanged proof content. Do not accept `--predecessor-head` when all heads still equal the assessment head.

For a staged continuation, require `--predecessor-head` and allow the heads to differ from the assessment head only through this chained contract: local `HEAD`, its upstream branch, and a fresh GitHub `headRefOid` must equal the supplied predecessor; the predecessor must descend from the assessment head without merge commits; and the complete PR comment stream must contain a trusted `<!-- mach12-progress -->` comment authored by the authenticated login with `OWNER`, `MEMBER`, or `COLLABORATOR` association that records the same review and assessment IDs/digests, `P`, original `V`, exact `prior_head`, exact `successor_head`, selected IDs, remaining staged IDs, ownership groups, and unchanged proof paths, node IDs, and content identities. Require the current `--predecessor-head` to equal the latest trusted record's `successor_head`, and require each earlier record's `successor_head` to equal the next record's `prior_head`. Walk backward through those trusted progress records until the authenticated assessment head is reached, rejecting gaps, forks, duplicate successors, reordered or split ownership groups, or any commit outside the recorded chain. At the current predecessor, require a clean index and tracked and untracked worktree; verify every remaining staged proof is still committed unchanged and every previously selected node remains at its authenticated path and node ID. If any ownership, linkage, marker, digest, identity, ancestry, content, or worktree invariant fails, stop before executing a proof or editing any file and report the workflow incomplete. Ordinary static-review comments continue through the ordinary path and must never activate this contract merely because arbitrary IDs were supplied.
...
1. Extract the exact retained node IDs, proof constraints, committed proof paths and content identities, and ownership groups for every surviving finding from both comments.
2. Run every retained node associated with the selected findings and reproduce its authenticated red state before editing production code. If a node is missing, stale, changed, or does not reproduce the recorded failure, stop and report the mismatch rather than adapting the proof.
3. Partition the applicable disposition domain—every surviving proof for a first repair, or only the predecessor's remaining staged proofs for a continuation—into findings selected now, explicitly staged for a named later repair, and explicitly declined for cleanup. Require a pairwise-disjoint exhaustive partition of that domain and one disposition per inseparable ownership group. Preserve selected and staged-later committed proof content unchanged.
4. Preserve the selected proofs' behavioral contracts, assertions, paths, node IDs, and test content. Prohibit weakening assertions, skipping or converting tests to expected failures, accepting snapshots, renaming or relocating paths or node IDs, and duplicating proof tests.
5. Change production code only to address the selected findings; make the selected committed nodes green in place. Unselected staged-later proofs may remain red.
6. Rerun the same selected retained nodes after the production edits and confirm they are green before running broader focused suites and delegating the production-only commit and push to `/mach12:push`.

For `--cleanup-finding`, require cleanup IDs to exhaust every surviving proof and reject any production-repair or staged-later disposition. Perform authentication and red-state reproduction exactly as above, then remove only complete authenticated ownership groups with targeted edits. Make no production change. Verify each removed node is gone and unrelated or surviving tests are unchanged, then delegate one targeted tests-only cleanup commit and push through `/mach12:push`. If no red proof survives, use the existing pre-merge-only completion route below.
...
Once the fixes are complete, capture the exact pre-commit head. For validation-origin repair or cleanup, also hash the exact complete bounded-operation patch bytes and include that SHA-256 in the structured payload. Then commit, push, and post a progress comment on the PR by delegating to:

```
/mach12:push
```

Pass a brief summary of the findings addressed as the arguments so the commit message and PR progress comment speak specifically to the fixes. For an ordinary static-review repair, also pass the exact resolved numeric review comment ID and instruct `/mach12:push` to preserve it as the originating review ID in the progress comment. For validation-origin repair, root the predecessor chain at the authenticated committed assessment head and pass a structured provenance payload containing the repository/PR/head-branch identity, exact authenticated upstream ref and its remote's canonical repository identity, review and assessment IDs/digests, `P`, original `V`, assessment head, exact `prior_head` equal to the pre-commit head, selected IDs, remaining staged IDs, cleanup IDs, ownership groups, and unchanged proof paths, node IDs, content identities, and exact bounded-operation patch SHA-256. Git ancestry is the durable transition evidence; progress comments record dispositions without duplicating executable proof bodies. Instruct `/mach12:push` to append the pushed `successor_head` and preserve every supplied provenance field verbatim in the progress comment. Fetch the posted numeric comment and verify its trusted author/association, marker, exact fields, and current GitHub head before offering a staged continuation. If publication fails or is ambiguous after a successful push, preserve the exact pushed successor, independently reread local and remote heads, and reconcile exactly one progress artifact without recommitting, repushing, force-pushing, or blindly retrying a comment. Block onward routing until publication is verified.
