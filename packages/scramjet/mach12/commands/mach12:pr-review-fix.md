...
Finding-validation evidence establishes whether a selected defect is real; it does not by itself establish that a proposed correction fits the current system. Before structural design or mutation, establish current coverage of the correction's affected owners, dependencies, contracts, and consumers. Reuse supported evidence after checking freshness and task coverage separately against current source, relevant intervening changes, and applicable uncommitted work. Investigate changed or uncovered portions before dependent work without expanding the locked finding scope.
...
1. **Codebase exploration** -- read every file referenced by the selected findings and trace the relevant code paths. Use focused parent investigation when sufficient; dispatch `mach12:structural-mapper` for unresolved ownership, dependency, contract, or consumer questions, `mach12:code-explorer` for execution flow, algorithms, transformations, failures, or side effects, and `scramjet:command-set-explorer` for unresolved command journeys or context boundaries. Do not dispatch multiple roles for equivalent evidence. Verify current assumptions before deciding requirements or architecture.
