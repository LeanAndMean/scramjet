...
2. **Codebase exploration** -- when prior planning is sufficient, verify its structural evidence remains fresh and covers this stage's affected owners, dependencies, contracts, and consumers; reuse supported portions rather than rebuilding them. Investigate changed or uncovered portions before dependent work, accounting for relevant uncommitted changes. Use focused parent investigation when sufficient; dispatch `mach12:structural-mapper` for unresolved ownership, dependency, contract, or consumer questions, `mach12:code-explorer` for execution flow, algorithms, transformations, failures, or side effects, and `scramjet:command-set-explorer` for unresolved command journeys or context boundaries. Do not dispatch multiple roles for equivalent evidence. Read every file the exploration flags.
