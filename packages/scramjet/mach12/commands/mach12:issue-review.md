...
9. **Structural coherence**: For material runtime architecture, does the plan derive cohesive boundaries and dependencies from essential responsibilities; assign stable policies and invariants to authoritative owners; distinguish critical from replaceable mechanisms; and preserve core behavior when supporting mechanisms are removed or replaced? Locally plausible options that share a material authority, removability, or dependency-direction defect are a plan defect. Judge the proposed system rather than requiring named layers or preferred topology.
...
- Only recommend a route after the selected path's publication gate passes: recommend `mach12:issue-review` when a verified revised plan still warrants another review; recommend `mach12:issue-implement` when a revised plan was verified and addresses all blockers, when a required Proceed-as-is decision comment was verified, or when a no-Critical/Important path required no comment.
