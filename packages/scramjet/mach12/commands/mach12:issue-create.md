...
- **Plausible matches**: Show each candidate's number, title, state, URL, `createdAt`, and `updatedAt`. Before confidently classifying any candidate as a duplicate or recommending linkage, use `read_issue` for the candidate and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until its native issue object and comments segments are complete. Track which candidates were read completely. Compare successfully read candidates' claims and intended scope with the newly approved issue and, where material, current authoritative repository context. If a read fails, surface the failure and exclude that unread candidate from duplicate classification and every mention, comment, or linkage target unless a retry succeeds.
...
- **Comment on one existing issue instead**: ask the user to select exactly one successfully read issue; unread candidates must not be offered. Only after the user explicitly selects the target, prepare `Related context: <summary of the new finding or context>.` Include any intentional originating issue or PR relationship using the linkable syntax above. Immediately before posting, call `read_issue` with `include: ["artifact", "comments"]` in an earlier assistant tool round and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot. Then pass that exact prepared body to `add_issue_comment` for only the chosen issue and skip creation.
- **Skip**: create no issue and post no relationship comment.

For a clear duplicate's **Link to existing** choice, prepare the same `Related context:` form. Immediately before posting, call `read_issue` with `include: ["artifact", "comments"]` in an earlier assistant tool round and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot. Then pass that exact body to `add_issue_comment`, report the issue and comment URLs, and skip creation. If selected duplicate references change the body, always complete the complete-draft review and renewed approval before publication.
...
Proceed to metadata only after `create_issue` returns its verified canonical URL. Derive the issue number from that URL's validated artifact path when needed; the tool does not return the number separately in model-visible content. If creation fails, surface the complete tool error, apply no metadata, and do not retry creation. When the error says the mutation may have succeeded, preserve that ambiguity, direct the user to reread the issue list before any recovery attempt, and report a non-completed status.

Resolve `assign me` with `gh api user --jq .login`, requiring the command to succeed and return exactly one non-empty login. If resolution fails, report the verified issue URL and derived number plus the failed `assign me` resolution; do not apply unresolved assignee metadata, retry issue creation, or claim complete success, and report a non-completed status. Apply each user-requested or repository-standard label and assignee operation only after verified identity, guard every operation, and record its exact failure. If any metadata operation fails, report the verified issue URL and derived number plus the exact label or assignee operation that failed; do not retry issue creation or claim complete success, and report a non-completed status.
