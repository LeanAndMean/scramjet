...
Classify structural due diligence before dispatching other advisory subagents. Skip a structural packet only when current authority establishes that the work is clearly mechanical, its owner and location are unambiguous, and it changes no shared, exported, public, serialized, cross-owner, or dependency contract. Otherwise establish one task-scoped Current-State Structural Evidence Packet before any architect runs. Reuse supported portions of available evidence when freshness and task coverage are established separately against current source, relevant intervening changes, and affected dependencies and consumers; a command boundary, matching revision, or recent timestamp alone proves neither. Account for relevant uncommitted changes when applicable. Dispatch `mach12:structural-mapper` with the initial issue boundary, user context, repository guidance, supplied evidence, and exact verification or extension questions when focused isolation or compression is useful. If validating and transporting old evidence is more burdensome than focused authoritative reacquisition, reacquire it. Uncertainty selects focused mapping rather than an exhaustive scan. If Step 5 materially changes the task boundary, refresh only the affected packet portion before architecture, using the reserved call described below.

Then classify the affected work:

- **Command-only** work changes executable natural-language surfaces: command or agent Markdown, frontmatter, next-step or delegation contracts, tool scopes, prompt artifacts, command-facing documentation, or tests whose subject is model interpretation. Use `scramjet:command-set-explorer` only when command journeys, context boundaries, artifacts, side-effect owners, or complete journeys need compression beyond the packet.
- **Code-only** work changes runtime source or executable implementation tests. Use `mach12:code-explorer` only for focused behavior, data flow, algorithms, side effects, analogous features, or issue-specific constraints and edge cases not established by the packet.
- **Mixed** work uses both families with disjoint briefs and file/claim partitions. Do not ask either family to review the other's surface. Consolidate runtime exploration while preserving each selected lens's evidence and output.

The mapper replaces overlapping structural, architecture-layer, ownership, dependency, integration-boundary, and contract-consumer exploration; it is not added on top of those lenses. When a packet is supplied, explorers report contradictions or gaps but do not recreate it.
...
Across exploration, architecture, and runtime test design in Steps 4, 6, and 8, use a maximum of eight subagent calls across both families. Limit the initial selected pass to a maximum of seven subagent calls so the eighth call remains available for one narrow mapper refresh. This is a ceiling for code-heavy or mixed work, not a target. The reserved call may refresh a materially changed task boundary before architecture or an exact evidence gap returned by an architect; spend it at most once. No rerun or decision branch may exceed the total ceiling.

Every brief must carry the task-relevant issue title, body, discussion, requirements, and decisions from Step 2; user context, project planning requirements, and verified project-tool commands, outputs, and limitations relevant to the specialist; relevant parent-established observations; the exact command/runtime partition; the selection reason; and the expected output. When a packet exists, pass its citations and every evidence limit. Compression may remove narration but must preserve responsibilities, dependencies, contracts, consumers, public exposure, unresolved gaps, and source references. Pass concise excerpts or summaries rather than an indiscriminate transcript. Record every selected agent and its evidence-based reason in the synthesis. A better-fit installed agent may replace an advisory role only when this command explicitly names it and defines the required output, or authoritative repository or command guidance establishes compatibility with the responsibility, read-only posture, context needs, required output, and workflow handoff. A catalog-only name or description match is supplementary and cannot replace an applicable named Scramjet specialist or exact Mach 12 role.
...
When a new or refreshed mapper packet is required, dispatch the mapper first. Otherwise verify the reusable packet before dependent work. Then dispatch the remaining selected exploration tasks in one parallel batch. Include user context and project planning requirements in each relevant brief. Each exploration should return key files and cited observations; after it completes, read every identified file needed to build deep understanding.
...
Based on the codebase findings and clarified requirements, route architecture by the Step 4 classification while preserving the shared eight-call ceiling and reserved refresh slot. For material code architecture, establish one common responsibility model before selecting components, interfaces, or files: essential product responsibilities and stable policies or invariants, authoritative owners, criticality and replaceability, and intended dependency direction. Keep its topology and vocabulary contextual; do not require named layers, a diagram, or a matrix. Carry this model in every code-architect brief so each option derives its implementation structure from the same system responsibilities.

When structural mapping was required, every architect brief must receive the Current-State Structural Evidence Packet, its citations, and every evidence limit before proposing files or interfaces. Require each architect to test every proposed location against the packet's owning responsibility, describe every proposed interface delta, enumerate known affected consumers, classify impact as `compatible`, `needs migration`, or `breaking`, include required in-repository migrations, and preserve unknown external exposure through compatibility, migration, or a user-owned decision.
...
After all results return, when material code architecture established a common responsibility model, compare every code option against it before presenting trade-offs. Discard an option with a material ownership, criticality, replaceability, or dependency-direction defect; if every option shares one, correct the architecture rather than asking the user to choose among structurally equivalent defects. In particular, reject designs where replaceable supporting mechanisms become authorities or required dependencies of essential behavior merely to reuse implementation logic. Otherwise retain the existing minimum-sufficient comparison without manufacturing a responsibility artifact.

Review the remaining approaches and form your own recommendation based on the issue's scope, the codebase's conventions, and the user's clarified requirements. For packet-backed work, an architect that proposes a location or contract outside the established boundary must return its supported analysis plus the exact evidence gap rather than claim a complete blueprint. The parent may spend the reserved eighth call on the same `mach12:structural-mapper` for one narrow refresh, then validate the existing proposals against the refreshed evidence. Do not automatically re-dispatch an architect. If the refresh invalidates a proposal and another specialist call is required, or the reserved call is unavailable, report incomplete evidence rather than draft or exceed the ceiling. Gate durable plan drafting on complete structural fit and contract-impact treatment.
...
- Recommend `mach12:issue-review` when the plan is non-trivial, touches risky areas, or should receive an approval gate; recommend `mach12:issue-implement` when the plan is small, uncontroversial, and you are confident in the staged breakdown.
