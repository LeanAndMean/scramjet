...
Classify structural due diligence before dispatching other advisory subagents. Skip the mapper only when current authority establishes that the work is clearly mechanical, its owner and location are unambiguous, and it changes no shared, exported, public, serialized, cross-owner, or dependency contract. Otherwise dispatch `mach12:structural-mapper` with the initial issue boundary, user context, repository guidance, and known evidence. It owns one task-scoped Current-State Structural Evidence Packet before any architect runs. Uncertainty selects focused mapping rather than an exhaustive scan. If Step 5 materially changes the task boundary, refresh only the affected packet portion before architecture, using the reserved call described below.
...
When mapping is required, dispatch the mapper first. Then dispatch the remaining selected exploration tasks in one parallel batch. Include user context and project planning requirements in each relevant brief. Each exploration should return key files and cited observations; after it completes, read every identified file needed to build deep understanding.
