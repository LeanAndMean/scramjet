...
Immediately before posting, reread the selected target in an earlier assistant tool round: use `read_pr` or `read_issue` with `include: ["artifact", "comments"]`, continuing every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot. Pass the exact prepared body to `add_pr_comment` or `add_issue_comment` respectively and record the verified canonical URL.
