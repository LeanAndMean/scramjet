...
Use `read_pr` and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until the native PR object and top-level comments segments are complete. Match the exact opaque `--review-comment` ID once in that verified stream and verify its repository, PR, trusted author, and `<!-- mach12-review -->` marker. Identify linked requirements from the complete PR evidence, use `read_issue` for each linked issue, and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until its native object and comments segments are complete. Use the current session, relevant implementation, and tests rather than treating the preliminary review's conclusions as true.
