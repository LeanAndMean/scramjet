...
For mixed work, run the runtime assessor first, then give its accepted set and root-cause conclusions to the command assessor strictly as non-verdict aggregate context. The command assessor must not reclassify runtime candidates; it assesses only its disjoint command candidates and owns one combined-system judgment across the merged accepted set. The parent validates completeness and identifiers, then mechanically merges complete verdicts unchanged, including their assessor-owned root-cause conclusions.
