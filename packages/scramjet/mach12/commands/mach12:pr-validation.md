...
Assign every retained finding to its permanent repository-relative test path, exact node ID, classification, and exactly one ownership-group ID. Require every retained finding that shares a proof path to use the same ownership-group ID, so each proof path belongs to exactly one indivisible ownership group; merge otherwise separate groups when they share a path or shared imports, fixtures, helpers, setup, or assertions. For each proof path, compute a provisional lowercase SHA-256 of its exact normalized worktree bytes. After the proof commit is created, recompute the authoritative proof content identity from the exact committed blob bytes at `V`; require it to match the committed path content and use only that post-commit identity in the review artifact. Require the path/node/content-identity/finding/group mapping to be exhaustive and non-overlapping, and require the complete normalized worktree diff to consist exactly of the declared proof paths.
...
- frozen implementation parent, proof commit, exact test path, node ID, per-path proof content SHA-256, and ownership-group ID (the finding ID itself when unshared);
