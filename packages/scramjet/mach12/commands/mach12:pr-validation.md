...
Use `read_pr` with `include: ["artifact", "comments", "files", "commits"]` and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until all four native segments are complete. Identify linked issues from that evidence; use `read_issue` for each one and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot until its native object and comments segments are complete. Read the approved plan and later amendments, changed production behavior, adjacent existing tests, PR-authored tests, and relevant prior review or fix artifacts.
...
Immediately before posting, call `read_pr` with `include: ["artifact", "comments"]` in an earlier assistant tool round and continue every returned segment window with its returned `include`, `offset`, optional `byte_offset`, and unchanged snapshot. Pass the exact prepared body to `add_pr_comment` and capture its verified canonical URL and, for this GitHub-only handoff, the numeric comment ID from that URL. Verify the exact stored comment belongs to the PR, has the expected marker and body, and was posted by the authenticated trusted user. If the tool reports an ambiguous write, use a fresh complete `read_pr` to reconcile the exact body and never blindly retry. Do not normalize tests into final suites, design production fixes, commit, or push in this command.
