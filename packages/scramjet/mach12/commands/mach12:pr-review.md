...
Before dependent reviewers are dispatched, establish current structural coverage when the combined PR materially changes or questions responsibilities, dependencies, contracts, or consumers. Reuse supported portions of available plans, packets, or prior review evidence only after checking freshness and task coverage separately against the checked-out source, relevant intervening changes, affected dependencies and consumers, and applicable uncommitted work. Investigate changed or uncovered portions with focused parent work or a bounded `mach12:structural-mapper` when isolation or compression is useful. Use `mach12:code-explorer` for unresolved execution flow, algorithms, transformations, failures, and side effects, and `scramjet:command-set-explorer` for unresolved command journeys or context boundaries. Do not dispatch multiple roles for equivalent evidence. Mapping remains descriptive and does not pre-classify findings or endorse corrections.
...
Dispatch at most seven subagents across structural evidence, context compression, and finding review, primarily for code-heavy mixed work. A mapper or explorer counts within this ceiling and does not become another finding reviewer. Command-only work uses one finding reviewer. If an explorer is needed, run it first, then dispatch the finding reviewers in one parallel call. All subagents are read-only; the parent owns tooling, synthesis, interaction, and publication.

Give each reviewer a focused brief containing the PR and changed surface, task-relevant issue authority and decisions, exact surface partition, selection reason, selected structural facts with citations, scope, source-state context, uncertainties and uncovered questions, parent-established observations, verified CI/project-tool evidence and limitations, user context, freshness caveats, expected cited output, and any claimed coaching/exception evidence plus the exact context presented before user approval. Do not ask reviewers to rerun project tools. If a version bump or changelog entry is present before pre-merge, flag it as premature.
...
- Assessment-relevant structural evidence, when established, as a concise descriptive handoff separate from F/S findings, retaining citations, scope, source-state context, uncertainties, and uncovered questions.
