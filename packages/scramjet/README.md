...
Scramjet provides eight independently allowlistable tools for current-repository issue and pull-request content: `read_issue`, `edit_issue`, `create_issue`, `add_issue_comment`, `read_pr`, `edit_pr`, `create_pr`, and `add_pr_comment`. Aggregate reads expose filtered native `gh`/`glab` JSON command transcripts, with exact segment subsets, fetch-once item continuations, and per-segment evidence snapshots. Ctrl+O expands a control-safe compound view: the reversible raw transcript remains in scrollback, while issue/PR and comment bodies render as Markdown and every retained non-body provider field remains visible as indented native JSON. Malformed or byte-fragmented results fail closed to raw-only display; mutations return only the verified canonical URL. Structured failures distinguish no-write paths from ambiguous writes and support deliberate read-only CLI recovery without automatic fallback or unsafe retry.
