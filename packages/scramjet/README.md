...
Drag to select displayed text, then right-click or press Ctrl+C to copy. Selection holds the presentation with a pending-update notice until copied or cleared. Without selection, Ctrl+C keeps its normal behavior. Alt+PageUp/Alt+PageDown enter transcript browsing from the tail; while detached, PageUp/PageDown/Home navigate and End/Escape return to live output. Focused selector bindings and overlays retain their input precedence. Terminal-native menus cannot see application selection, and iTerm2 requires its existing right-click-reporting preference for right-click copy.
