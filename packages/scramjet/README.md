...
The interactive transcript uses Scramjet's retained viewport and **rightmost scrollbar**, not the terminal's native history. Wheel/trackpad scrolling and scrollbar dragging can reach running tool/subagent cards before a turn finishes; unrelated updates preserve your reading position. The editor and footer scroll with the document, and typing reveals the editor again.

Drag to select displayed text, then right-click or press Ctrl+C to copy. Selection holds the presentation with a pending-update notice until copied or cleared. Without selection, Ctrl+C keeps its normal behavior. While browsing away from the tail, PageUp/PageDown/Home navigate and End/Escape return to live output; focused overlays retain keyboard precedence. Terminal-native menus cannot see application selection, and iTerm2 requires its existing right-click-reporting preference for right-click copy.
