...
The existing gap above the dock displays `Session: N lines below · Ctrl+End: latest` while session rows remain below the viewport; it stays blank at the tail without adding a horizontal rule. The count excludes the dock and is not an unread-message or hidden-draft count. Narrow layouts shorten the hint, retaining **Session**, with `Session ↓…` when an exact count cannot fit; application copying excludes this decoration.

Structured confirmation, choice and next-step selectors use paired settings-style rules with their title, options and existing controls inside. Their visible window follows item selection by rendered rows, allowing partial neighboring choices for orientation. The window moves only as needed to reveal the selected choice; the existing option-count and field-length caps remain. Genuine insufficient space for one selected representation plus required controls retains explained dock suspension rather than silently dropping content or changing acceptance behavior. Passive widgets and footer information remain outside the frame.

Drag to select displayed text, then right-click or press Ctrl+C to copy. Selection does not freeze output or waiting controls; Copy captures the last displayed highlight before asynchronous clipboard delivery. Drags can cross between transcript and editor, with downward transcript autoscroll reaching the bottom first. Built-in copying rejoins soft wraps, including in the editor. Without selection, Ctrl+C keeps its normal behavior and right-clicking the focused editor pastes local clipboard text without submitting. Alt+PageUp/Alt+PageDown enter transcript browsing from the tail; while detached, PageUp/PageDown navigate and Escape returns to live output. Home/End retain editor line-start/end behavior; Ctrl+Home goes to the transcript beginning and Ctrl+End returns to its bottom and resumes following at any scroll position. Focused selector bindings and overlays retain their input precedence. Terminal-native menus cannot see application selection, and iTerm2 requires its existing right-click-reporting preference for right-click copy.
