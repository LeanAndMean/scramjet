...
After each publish and during final verification, read-only polling uses one ten-minute elapsed observation allowance per package. The allowance includes registry-command latency and retry sleeps; each command timeout and sleep is capped to the time remaining. Exhaustion, malformed state, or ambiguous transport stops the release permanently. It does not authorize another workflow attempt or same-tag continuation; inspect state read-only, correct repository code or external configuration as needed, and prepare another five-fresh release under a new tag. The allowance tolerates the observed delay without attributing it to npm processing, registry propagation, or client behavior.

A successful `npm publish` return establishes only that the publish command was accepted. The package is reported as observed only after its exact version, `latest`, preserved non-`latest` tags, and attestation URL pass the shared post-publish gate. Publication success leaves final verification pending. Publication failure output identifies accepted-and-observed packages, any accepted-but-unobserved or acceptance-ambiguous package, later unattempted packages, the failed phase, and incomplete final verification. Final verification is completed only after the separate metadata/provenance, exact-installation, installed-closure, signature-audit, installed-runtime, and CLI checks all pass; its failure output identifies the metadata observed during that verification invocation, the failed metadata or consumer phase, and incomplete final verification without implying unknown publication acceptance. This invocation-local evidence is never persisted or used to authorize publication.
...
Do not rerun these workflows, move or recreate these tags, rewrite these releases, reconcile retained runtime artifacts for continuation, continue a partial package set under the same tag, or publish a missing package into one of these releases. The sole recovery path is read-only inspection followed by another release with five fresh versions and a new tag.
