...
4. Set `TAG=v<scramjet version>` and create the single normal release with `gh release create "$TAG" --target "$MERGED_SHA"`. Fetch that exact remote tag and require `git rev-list -n 1 "$TAG"` to equal `MERGED_SHA` before allowing publication to continue. Verify its workflow event ref, SHA, workflow ref, Node/npm versions, complete preflight, dependency order, OIDC authentication, and dist-tag effects.
