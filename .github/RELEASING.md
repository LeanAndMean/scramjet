...
A successful `npm publish` return establishes only that the publish command was accepted. The package is reported as observed only after its exact version, `latest`, preserved non-`latest` tags, and attestation URL pass the shared post-publish gate. Publication success leaves final verification pending. Final verification is completed only after the separate metadata/provenance, exact-installation, installed-closure, signature-audit, and CLI checks all pass. Failure output identifies accepted-and-observed packages, any accepted-but-unobserved or acceptance-ambiguous package, later unattempted packages, the failed phase, and incomplete final verification; this invocation-local evidence is never persisted or used to authorize publication.
