...
| `packages/coding-agent/src/core/model-registry.ts` | Extended compat schemas with the complete known OpenAI/Responses/Anthropic field sets, including Anthropic `supportsTemperature` and `forceAdaptiveThinking`; made provider compat union branches strict; added effective-API compat validation | Custom model schema accepts the new compat fields while rejecting invalid known fields and API-incompatible fields that previously matched unrelated permissive union branches or were silently ignored ([#245](https://github.com/LeanAndMean/scramjet/issues/245)) |
