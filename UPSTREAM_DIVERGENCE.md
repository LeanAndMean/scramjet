...
| `packages/coding-agent/src/core/extensions/types.ts` | Added `preTurnMessages` to `BeforeAgentStartEventResult`; added `PrepareNextTurnEvent`, `PrepareNextTurnEventResult` interfaces; added `prepare_next_turn` to `ExtensionEvent` union and `ExtensionAPI.on()` overloads | Extension hooks for synthetic message injection before LLM calls ([#238](https://github.com/LeanAndMean/scramjet/issues/238)) |
| `packages/coding-agent/src/core/extensions/runner.ts` | Added `preTurnMessages` to `BeforeAgentStartCombinedResult`; collect `preTurnMessages` in `emitBeforeAgentStart()`; added `PrepareNextTurnCombinedResult` and `emitPrepareNextTurn()` | Runtime plumbing for prompt-start and intra-run message injection ([#238](https://github.com/LeanAndMean/scramjet/issues/238)) |
| `packages/coding-agent/src/core/agent-session.ts` | Added `_drainAgentEventQueue()` helper; set `this.agent.beforeToolBatch` to drain queue; refactored existing `beforeToolCall` to reuse helper; inject `preTurnMessages` in `prompt()` flow; wire `agent.prepareNextTurn` to emit `prepare_next_turn` extension event with message persistence and context snapshot | Pre-extraction queue drain ([#196](https://github.com/LeanAndMean/scramjet/issues/196)); prompt-start and intra-run extension message injection ([#238](https://github.com/LeanAndMean/scramjet/issues/238)) |
