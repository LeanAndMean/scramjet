...
| `packages/agent/src/agent.ts` | Added `Agent.runHarnessTool` (transient idle run + mid-run queue with turn-boundary and end-of-run flush drains); `createLoopConfig().prepareNextTurn` always runs (drains harness queue + routing self-heal onto `state.model`); `prepareNextTurn` public signature widened to receive the live `PrepareNextTurnContext`; `processEvents` falls back to a transient run signal; `reset()` warns when it discards a non-empty harness tool queue | Harness-tool-invocation primitive: model-change notices and other harness-originated tool calls execute with real event/persistence/routing semantics but no run/turn framing ([#244](https://github.com/LeanAndMean/scramjet/issues/244)) |
| `packages/agent/src/agent.ts` | `runHarnessTool` now returns a deferred promise that settles on actual Agent-core execution (resolve after the pipeline completes; reject on reset/teardown revocation, escaping infrastructure/listener/hook failure, or a failed final drain that strands later queued calls) rather than resolving immediately when queued; added an all-unsettled invocation set, `rejectUnsettledHarnessTools()`, and final-drain remnant rejection; broadened the `reset()` warning from queued to unsettled calls; exported `generateHarnessToolCallId` | Makes the harness-tool promise truthfully order execution before a consumer proceeds; a tool's own `execute()` error still resolves as an `isError` result ([#341](https://github.com/LeanAndMean/scramjet/issues/341)) |
...
| `packages/coding-agent/src/core/system-prompt.ts` | Product identity in `buildDefaultCorePrompt` template (6 refs: Pi → Scramjet); added `sessionFile` to `BuildSystemPromptOptions` and conditional `Current session journal` line in the volatile tail | Agent system prompt references the correct product name; session journal path exposed as environment fact ([#348](https://github.com/LeanAndMean/scramjet/issues/348)) |
...
| `packages/coding-agent/src/core/agent-session.ts` | `_rebuildSystemPrompt` passes `sessionManager.getSessionFile()` as `sessionFile` in `BuildSystemPromptOptions` | Wires live session file path to the volatile environment fact ([#348](https://github.com/LeanAndMean/scramjet/issues/348)) |
...
| `packages/coding-agent/src/core/extensions/types.ts` | Documented the `ExtensionAPI.invokeHarnessTool` persisted-settlement contract in JSDoc (resolves only after the tool-result is persisted; rejects on unknown tool, matching-event persistence failure, disposal, or a duplicate pending explicit id; rejection is not artifact absence) | The API's public contract now promises persisted settlement, not just Agent-core execution ([#341](https://github.com/LeanAndMean/scramjet/issues/341)) |
...
| `packages/coding-agent/src/core/agent-session.ts` | `invokeHarnessTool` layers a persisted-settlement boundary on `Agent.runHarnessTool`: allocates a tool-call id (rejecting a duplicate pending explicit id), registers a tool-call-id-keyed persistence acknowledgement, and awaits Agent execution + acknowledgement via `Promise.all` with identity-checked map cleanup; `_handleAgentEvent` routes through a `_processAgentEventTracked` wrapper that resolves the acknowledgement after the matching tool-result `message_end` is persisted and rejects it if processing a matching event throws; `dispose()` is now idempotent (`_disposed` guard), rejects pending acknowledgements, and calls `Agent.rejectUnsettledHarnessTools()` | Consumers can `await` until the harness-tool record row is persisted before replacing/tearing down the session; disposal revokes in-flight invocations instead of stranding them ([#341](https://github.com/LeanAndMean/scramjet/issues/341)) |
...
| `packages/coding-agent/src/core/extensions/runner.ts` | Added private `skipStale()` guard; every `emit*` method (all ten: `emit`, `emitMessageEnd`, `emitToolResult`, `emitToolCall`, `emitUserBash`, `emitContext`, `emitBeforeProviderRequest`, `emitBeforeAgentStart`, `emitResourcesDiscover`, `emitInput`) checks it at method top and before each handler invocation, returning silently when `staleMessage` is set; a skipped `session_shutdown` is routed to `emitError` instead of dropped (ordering-violation signal) | A deferred fresh-session dispatch (`ctx.newSession()` via `setTimeout(0)`) can invalidate the runner while an `agent_end` emit is suspended at an earlier handler's await; the resumed emit then invoked later handlers against a dead ctx, surfacing a stale-ctx error. Semantic change: handlers that used only the event payload previously ran to completion after invalidation and are now skipped. `reload()` swaps runners without disposing the session — queued events resolve `this._extensionRunner` at execution time and land in the new live runner, so that path needs no guard; genuine stale-ctx misuse (a handler touching a captured ctx after an await, post-invalidation) still throws via `assertActive` ([#326](https://github.com/LeanAndMean/scramjet/issues/326)) |
