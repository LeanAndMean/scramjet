...
| `packages/coding-agent/src/index.ts` | Exported `loadThemeFromPath`, `resolveThemeName`, `getCurrentThemeName` from the theme module | Public API so scramjet (and downstream extensions) can load themes from a path and resolve/inspect the active theme ([#322](https://github.com/LeanAndMean/scramjet/issues/322)) |
